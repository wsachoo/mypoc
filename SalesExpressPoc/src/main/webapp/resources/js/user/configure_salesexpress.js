/*$(document).ready(function() {
	var $accessSpeedConfigPlaceholder = $("#accessSpeedConfigPlaceholder"); 

	$("input[name='accessRequired']").on("change", function() {
		var isAccessRequiredValue = $(this).val();
		if ('true' === isAccessRequiredValue) {
			var optionButtons = $.tmpl("select_speed_configure_options", avpnJsonData);
			var lastDiv = $accessSpeedConfigPlaceholder.find('div.row:last');
			lastDiv.after(optionButtons);
		} else {
			removeNextAllSiblingDivRows($(this));
		}
	});
	
	$accessSpeedConfigPlaceholder.on("click", "input[name='filteredAccessTypes']", function(e) {
	...
	lastDiv = $accessSpeedConfigPlaceholder.find('div.row:last');
	...
	$accessSpeedConfigPlaceholder.on("change", "#selectAccessType", function(e) {
		var $divSelectedAccessTypes = $("#divSelectedAccessTypes");
		var $selectAccessType = $("#selectAccessType");
		...
		if (!($selectAccessType == undefined)) {
			...
			if (selectAccessTypeLength == 1) {
				...
				var $selectedAccessTypes = $divSelectedAccessTypes;
				$.each( selectedValues, function( key, value ) {
					$selectedAccessTypes.append("<input type='radio' class='form-radio-input' name='radiofilteredAccessTypes' id='filteredAccessTypes_" + key + "' value='" + key + "'>");
					$selectedAccessTypes.append(value);
					$selectedAccessTypes.append("&nbsp;&nbsp;");
				});
			}
